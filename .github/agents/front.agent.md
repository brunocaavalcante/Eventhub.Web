...
16. [Testes Unitários](#testes-unitários)
17. [Checklist de Criação](#checklist-de-criação)
18. [Anti-Patterns](#anti-patterns)
19. [Setup Inicial do Projeto](#setup-inicial-do-projeto)
...
### ⚠️ IMPORTANTE: Mock Obrigatório do UsuarioService

Como `BaseComponent` injeta `UsuarioService`, **SEMPRE** inclua este mock nos testes para evitar `NullInjectorError`.
...
describe('MeuComponente', () => {
  ...
  beforeEach(async () => {
    // Mock OBRIGATÓRIO do UsuarioService
    mockUsuarioService = {
    ...
    await TestBed.configureTestingModule({
      imports: [MeuComponente, NoopAnimationsModule, HttpClientTestingModule],
      providers: [
  ...
  it('deve validar campo obrigatório', () => {
    const campo = component.form.get('nome');
    campo?.setValue('');
    expect(campo?.hasError('required')).toBe(true);
  });

  it('deve chamar serviço', () => {
    const mockService = { salvar: jest.fn().mockReturnValue(of({ sucesso: true })) };
    component.salvar();
...
beforeEach(async () => {
  ...
  mockUsuarioService = {
    obterUsuarioLogado: jest.fn().mockResolvedValue({ id: 1, nome: 'Admin' })
  };

  await TestBed.configureTestingModule({
    imports: [MeuModal, NoopAnimationsModule],
    providers: [
      { provide: MatDialogRef, useValue: mockDialogRef },
      { provide: MAT_DIALOG_DATA, useValue: mockData },
      { provide: UsuarioService, useValue: mockUsuarioService } // OBRIGATÓRIO
    ]
  }).compileComponents();
});
```
...
✅ **SEMPRE mockar:** `UsuarioService` (obrigatório), serviços injetados, `Router`, `ActivatedRoute`  
✅ **SEMPRE importar:** `NoopAnimationsModule`, `HttpClientTestingModule`  
✅ **Nomenclatura:** Português (`deve criar componente`, `deve validar campo`)  
✅ **Cobertura mínima:** Criação, validações, chamadas de serviço, erros  
✅ **Mocks:** `jest.fn()` síncronos, `mockResolvedValue()` para async  

❌ **Evitar:** Testar implementação interna, mocks complexos, console.log nos testes  
...
- [ ] **Criar testes unitários:**
  - [ ] Importar `NoopAnimationsModule` e `HttpClientTestingModule`
  - [ ] **SEMPRE criar mock de `UsuarioService`** (obrigatório para BaseComponent)
  - [ ] Mockar services injetados no componente
  - [ ] Testar criação, validações e chamadas de serviços
  - [ ] Nomenclatura em português: `deve criar componente`
...
✅ **Criar testes unitários com mock de `UsuarioService`** (obrigatório)  
✅ Importar `NoopAnimationsModule` nos testes  
...
❌ **Esquecer de mockar `UsuarioService` nos testes**  
